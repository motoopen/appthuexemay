/* motoai_v37_retrieval.js
   Messenger UI + SmartContext-Deep + VN-only + Auto-avoid
   + AutoLearn Retrieval (RAG)
   - Giọng tự nhiên: loại "ạ/nhé/nha" và các hạt tương tự
*/
(function(){
  ...
  // Natural tone: loại bỏ hạt “ạ/nhé/nha/ạ.”, gọn câu
  function naturalize(t){
    ...
    // đảm bảo kết thúc bằng . nếu là chữ
  ...
  function renderSess(){
    ...
    else addMsg('bot', naturalize(`Xin chào 👋, em là nhân viên hỗ trợ của ${CFG.brand}. Anh/chị muốn xem Xe số, Xe ga, Xe điện hay Thủ tục thuê xe?`));
  }
  ...
  // ===== Deep Compose (giọng tự nhiên)
  const PREFIX = ["Chào anh/chị,","Xin chào 👋,","Em chào anh/chị,","Rất vui được hỗ trợ anh/chị,"];
  const SUFFIX = [".", ". Cảm ơn anh/chị."];
  const pick = a => a[Math.floor(Math.random()*a.length)];

  function polite(s){
    s=(s||"").trim();
    if(!s) s="em chưa nhận được câu hỏi, anh/chị thử nhập lại giúp em.";
    return naturalize(`${pick(PREFIX)} ${s}${pick(SUFFIX)}`);
  }

  // ===== NÂNG CẤP v37: Chuyển deepAnswer thành async để chờ retrieval
  async function deepAnswer(userText){
    ...
    if(it.needContact) return polite(`anh/chị có thể nhắn Zalo ${CFG.phone} hoặc gọi ${CFG.phone} để được hỗ trợ nhanh.`);
    if(it.needDocs) return polite(`thủ tục gọn: CCCD/hộ chiếu + cọc tuỳ xe. Có phương án giảm cọc khi đủ giấy tờ.`);
    if(it.needPrice) return composePrice(type, qty);

    if(/(chào|xin chào|hello|hi|alo)/i.test(lower)){
      return polite(`em là nhân viên hỗ trợ của ${CFG.brand}. Anh/chị muốn xem Xe số, Xe ga, Xe điện hay Thủ tục thuê xe?`);
    }

    // ===== NÂNG CẤP v37: Fallback sang Retrieval =====
    // Nếu không có intent nào khớp, thử tìm trong index đã học
    try {
      const top = searchIndex(userText, 3); // Lấy 3 kết quả
      if (top && top.length) {
        const bullets = top.map(t => {
          const titleOrText = (t.title || t.text || '').trim();
          const snippet = mkSnippet(titleOrText, userText, 140); // Cắt snippet
          // Lấy domain từ 'source' hoặc 'url'
          let domain = t.source || 'nguồn';
          try { if(t.url) domain = new URL(t.url).hostname.replace(/^www\./,''); } catch {}
          return `• ${snippet} (${domain})`;
        });
        // Trả lời tự nhiên, dùng giọng "Em"
        const reply = `Em tìm thấy một số thông tin liên quan:\n${bullets.join('\n')}\nAnh/chị cần em tóm tắt/tra rõ chỗ nào trong các nội dung trên không?`;
        return naturalize(reply); // Dùng naturalize để đồng bộ giọng
      }
    } catch(e) {
      console.warn('MotoAI v37 retrieval err', e);
    }

    // Fallback gốc: Nếu retrieval cũng không thấy
    return polite(`anh/chị quan tâm loại xe nào (xe số, Vision, Air Blade, 50cc, côn tay…) và thuê mấy ngày để em báo giá phù hợp.`);
    // ===== KẾT THÚC NÂNG CẤP v37 =====
  }

  function composePrice(type, qty){
    if(!type) type = 'xe số';
    if(!qty)  return polite(`Giá ${type} khoảng ${summariseType(type)}. Anh/chị muốn thuê mấy ngày để em ước tính tổng?`);
    const base = baseFor(type, qty.unit);
    if(!base)  return polite(`Giá theo ${qty.unit} của ${type} chưa có sẵn. Anh/chị nhắn Zalo ${CFG.phone} để có báo giá chính xác.`);
    const total = base * qty.n;
    const label = qty.unit==='ngày'?`${qty.n} ngày`:qty.unit==='tuần'?`${qty.n} tuần`:`${qty.n} tháng`;
    return polite(`Ước tính thuê ${type} ${label} khoảng ${nfVND(total)}đ. Cần em giữ xe hoặc tư vấn thêm không?`);
  }
  ...
  // ===== NÂNG CẤP v37: Thêm các hàm Retrieval từ patch =====
  
  // tokenize helper (simple) - hỗ trợ unicode
  function tk(s){ return (s||'').toLowerCase().replace(/[^\p{L}\p{N}\s]+/gu,' ').split(/\s+/).filter(Boolean); }

  // get flattened index (dùng hàm loadLearn() nội bộ)
  ...
  function searchIndex(query, k=3){
    ...
    const scored = idx.map(item=>{
      const text = ((item.title||'') + ' ' + (item.text||'') + ' ' + (item.url||'')).toLowerCase();
      let s=0; for(const t of qtok) if(text.includes(t)) s++;
      return Object.assign({score:s}, item);
    }).filter(x=> x.score>0).sort((a,b)=> b.score - a.score).slice(0,k);
    return scored;
  }

  // small helper to make snippet short
  function mkSnippet(text, q, maxLen=220){
    if(!text) return '';
    const t = text.replace(/\s+/g,' ').trim();
    if(t.length <= maxLen) return t;
    // try to find first occurrence of a query token
    const tokens = tk(q);
    for(const tok of tokens){
      const p = t.toLowerCase().indexOf(tok);
      if(p>=0){
        const start = Math.max(0, p-60);
        return (start>0? '...':'') + t.slice(start, Math.min(start+maxLen, t.length)) + (p+maxLen < t.length ? '...' : '');
      }
    }
    return t.slice(0, maxLen) + (t.length>maxLen ? '...' : '');
  }
  // ===== KẾT THÚC KHỐI RETRIEVAL v37 =====
  ...
  function clearChat(){ try{ localStorage.removeItem(K.sess); localStorage.removeItem(K.ctx); }catch{}; $('#mta-body').innerHTML=''; addMsg('bot', naturalize('Đã xóa hội thoại, em sẵn sàng hỗ trợ lại.')); }

  // NÂNG CẤP v37: sendUser là async, và AWAIT deepAnswer
  async function sendUser(text){
    ...
    // AWAIT ĐÃ ĐƯỢC THÊM Ở ĐÂY
    let ans = await deepAnswer(userText); 

    hideTyping(); addMsg('bot', naturalize(ans)); // ans đã được naturalize BÊN TRONG deepAnswer hoặc composePrice
  ...
  ready(async ()=>{
    ...
    console.log('%cMotoAI v37 Retrieval-Deep — Ready','color:'+CFG.themeColor+';font-weight:bold;');
  ...
  window.MotoAI_v37 = {
    ...
    // getIndex bây giờ dùng hàm nội bộ
    getIndex: ()=> { try{ return getIndexFlat(); }catch{ return []; } },
